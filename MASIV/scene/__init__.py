...
class Scene:
    ...
    def __init__(
        ...
        if os.path.exists(os.path.join(args.source_path, "sparse")):
            ...
            scene_info = sceneLoadTypeCallbacks["PacNeRF"](
                args.source_path, args.config_path, args.white_background, load_fix_pcd=load_fix_pcd, read_cam=read_cam
            )
        elif os.path.exists(os.path.join(args.source_path, "camera.json")) and os.path.exists(
            os.path.join(args.source_path, "frame.json")
        ):
            ...
            scene_info = sceneLoadTypeCallbacks["SpringGausMPMSynthetic"](
                args.source_path, args.config_path, args.white_background, args.num_frame
            )
        elif os.path.exists(os.path.join(args.source_path, "transforms_train.json")):
            print("Found transforms_train.json file, assuming Genesis data set!")
            ...
            scene_info = sceneLoadTypeCallbacks["SpringGausRealCapture"](
                args.source_path, args.white_background, args.eval, mode=args.mode
            )
        else:
    ...
    def save(self, iteration, fix_pcd=False, postfix=""):
        folder = "point_cloud" if not fix_pcd else "point_cloud_fix_pcd"
        folder = folder + postfix
        name = f"{folder}/iteration_{iteration}"
