...
from utils.general_utils import floor, ceil
...
def readSpringGausCaptureRealInfo(path, white_background, eval, mode="static"):
    root_dir = path.split("/")
    obj_name = root_dir[-1]
    ...
    shs = np.random.random((num_pts, 3)) / 255.0
    pcd = BasicPointCloud(points=xyz, colors=SH2RGB(shs), normals=np.zeros((num_pts, 3)))
    ply_path = os.path.join(root_dir, f"dynamic/sequences/{obj_name}/points3d.ply")
    storePly(ply_path, xyz, SH2RGB(shs) * 255)
    train_cam_infos = readSpringGausCaptureRealCameras(root_dir, obj_name, white_background, mode=mode)
...
def readSpringGausCaptureRealCameras(root_dir, obj_name, white_background, mode="static"):
...
def readPACNeRFInfo(path, config_path, white_background, eval_cam_id=0, load_fix_pcd=False, read_cam=True):
    ...
    if not load_fix_pcd:
        if "data" in cfg:
            ...
            xyz_min, xyz_max = np.inf, -np.inf
            for file in sorted(glob.glob(os.path.join(path, "point_clouds/*.ply"))):
                pcd = np.asarray(o3d.io.read_point_cloud(file).points)
                xyz_min, xyz_max = np.minimum(pcd.min(0), xyz_min), np.maximum(pcd.max(0), xyz_max)
            xyz_min, xyz_max = floor(xyz_min, decimal=1), ceil(xyz_max, decimal=1)
        bounds = xyz_max - xyz_min
        num_pts = np.prod(bounds * 64)
...
def readCamerasFromAllData(path, white_background):
    ...
    # TODO: remove all_data_full.json for Genesis
    if os.path.exists(os.path.join(path, "all_data_full.json")):
        # Genesis
        json_path = os.path.join(path, "all_data_full.json")
    else:
        # GIC
        json_path = os.path.join(path, "all_data.json")
