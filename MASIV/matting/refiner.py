...
class Refiner(nn.Module):
    ...
    def crop_patch(
        self, x: torch.Tensor, idx: Tuple[torch.Tensor, torch.Tensor, torch.Tensor], size: int, padding: int
    ):
        """
    ...
    def replace_patch(self, x: torch.Tensor, y: torch.Tensor, idx: Tuple[torch.Tensor, torch.Tensor, torch.Tensor]):
        """
    ...
    def compute_pixel_indices(
        ...
        o = torch.arange(O)
        idx_pat = (
            (c * H * W).view(C, 1, 1).expand([C, O, O])
            + (o * W).view(1, O, 1).expand([C, O, O])
            + o.view(1, 1, O).expand([C, O, O])
        )
