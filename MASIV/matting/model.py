...
class MattingRefine(MattingBase):
    ...
    def forward(self, src, bgr):
        ...
        src_sm = F.interpolate(
            src, scale_factor=self.backbone_scale, mode="bilinear", align_corners=False, recompute_scale_factor=True
        )
        bgr_sm = F.interpolate(
            bgr, scale_factor=self.backbone_scale, mode="bilinear", align_corners=False, recompute_scale_factor=True
        )
