...
class MPMStatics(Statics):
    ...
    def update_vol(self, sections: list[int], vols: list[float]) -> None:
        ...
        for section, vol in zip(sections, vols):
            wp.launch(
                self.set_float,
                dim=self.vol.shape,
                inputs=[self.vol, offset, offset + section, vol],
    ...
    def update_rho(self, sections: list[int], rhos: list[float]) -> None:
        ...
        for section, rho in zip(sections, rhos):
            wp.launch(
                self.set_float,
                dim=self.rho.shape,
                inputs=[self.rho, offset, offset + section, rho],
    ...
    def update_clip_bound(self, sections: list[int], clip_bounds: list[float]) -> None:
        ...
        for section, clip_bound in zip(sections, clip_bounds):
            wp.launch(
                self.set_float,
                dim=self.clip_bound.shape,
                inputs=[self.clip_bound, offset, offset + section, clip_bound],
                device=self.clip_bound.device,
            )
            offset += section
...
class MPMState(State):
    ...
    def from_torch_grad(
        ...
        grad_stress: Optional[Tensor] = None,
    ) -> None:
        """
...
class MPMModel(Model):
    ...
    def forward(
        self, statics: MPMStatics, state_curr: MPMState, state_next: MPMState, tape: Optional[Tape] = None
    ) -> None:
        device = self.device
        ...
        with CondTape(tape, self.requires_grad):
            wp.launch(
                self.g2p,
                dim=num_particles,
                inputs=[constant, statics, particle_curr, particle_next, grid],
                device=device,
            )
    ...
    @staticmethod
    @wp.kernel
    def p2g(constant: ConstantType, statics: StaticsType, particle_curr: MPMParticleData, grid: MPMGridData) -> None:
        p = wp.tid()
        ...
        f_x = p_x - wp.vec3(float(base_x), float(base_y), float(base_z))
    ...
    @staticmethod
    @wp.kernel
    def grid_op_freeslip(constant: ConstantType, grid: MPMGridData) -> None:
        px, py, pz = wp.tid()

        v = wp.vec3(0.0)
        if grid.m[px, py, pz] > constant.eps * (constant.dx**3.0):
            v = grid.mv[px, py, pz] / grid.m[px, py, pz] + constant.gravity * constant.dt
        else:
    ...
    @staticmethod
    @wp.kernel
    def grid_op_noslip(constant: ConstantType, grid: MPMGridData) -> None:
        px, py, pz = wp.tid()
    ...
    def g2p(
        ...
        new_x = wp.vec3(
            wp.clamp(new_x[0], 0.0 + bound, constant.size - bound),
            wp.clamp(new_x[1], 0.0 + bound, constant.size - bound),
            wp.clamp(new_x[2], 0.0 + bound, constant.size - bound),
        )
...
class MPMModelBuilder(ModelBuilder):
    ...
    def parse_cfg(self, cfg: DictConfig) -> "MPMModelBuilder":
        ...
        self.config["num_grids"] = int(num_grids * size)
        self.config["dt"] = dt
    ...
    def build_constant(self) -> ConstantType:
        ...
        constant.center = self.config["center"]
        constant.size = self.config["size"]
...
class MPMInitData(object):
    ...
    @staticmethod
    def alignment(min_bound_1: np.ndarray, max_bound_1: np.ndarray, min_bound_2: np.ndarray, max_bound_2: np.ndarray):
    ...
    @classmethod
    def get(cls, cfg: DictConfig, data: Any) -> "MPMInitData":
        kwargs: dict = None
        if cfg.shape.type == "gs":
            kwargs = cls.get_gs(cfg, data)
        elif cfg.shape.type == "pcd":
            kwargs = cls.get_pcd(
                cfg.shape.name,
                cfg.shape.asset_root,
                cfg.shape.sort,
                cfg.shape.get("ori_bounds"),
                cfg.shape.get("sim_bounds"),
            )
        elif cfg.shape.name.startswith("cube"):
            kwargs = cls.get_cube(
                cfg.shape.center,
                cfg.shape.size,
    ...
    @classmethod
    def get_gs(cls, cfg, data: Any) -> dict[str, Any]:
        p_x = data.get_xyz
        num_particles = p_x.shape[0]
        size = cfg.size * np.ones(3)
        vol = np.prod(size) / num_particles
        center = cfg.center * np.ones(3)
        return dict(num_particles=num_particles, vol=vol, pos=p_x, center=center, size=size)
    ...
    def get_pcd(
        ...
        if asset_root is None:
            asset_root = Path(__file__).resolve().parent.parent.parent.parent / "experiments" / "assets"
        else:
        ...
        if (asset_root / precompute_name).is_file():
            ...
            pcd_path = (asset_root / f"{name}.ply").as_posix()
            pcd: trimesh.PointCloud = trimesh.load(pcd_path)
            p_x = np.array(pcd.vertices).copy()

            if sort is not None:
                indices = np.array(list(sorted(range(p_x.shape[0]), reverse=True, key=lambda x: p_x[:, sort][x])))
                p_x = p_x[indices]

            mesh_path = [_ for _ in Path(pcd_path).parent.glob("mesh.*")]
            if len(mesh_path) == 1:
                mesh: trimesh.Trimesh = trimesh.load(mesh_path[0].as_posix(), force_mask=True)
                vol = mesh.volume / p_x.shape[0]
            else:
                vol = pcd.convex_hull.volume / p_x.shape[0]
                print(f"  WARNING: mesh file not found, using convex hull volume.")
        ...
        return dict(num_particles=p_x.shape[0], vol=vol, pos=p_x, center=center, size=size)
    ...
    @classmethod
    def get_cube(cls, center: list | np.ndarray, size: list | np.ndarray, resolution: int, mode: str) -> dict[str, Any]:
        asset_root = Path(__file__).resolve().parent.parent / "assets"
