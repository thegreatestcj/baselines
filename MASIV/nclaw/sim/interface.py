...
class MPMSimFunction(autograd.Function):
    ...
    def backward(
        ...
        grad_C_next: Tensor,
        grad_F_next: Tensor,
    ) -> tuple[None, None, None, None, Tensor, Tensor, Tensor, Tensor, Tensor]:
        ...
        state_next.from_torch_grad(grad_x=grad_x_next, grad_v=grad_v_next, grad_C=grad_C_next, grad_F=grad_F_next)
...
class MPMDiffSim(MPMSim):
    def forward(
        self, x: Tensor, v: Tensor, C: Tensor, F: Tensor, stress: Tensor
    ) -> tuple[Tensor, Tensor, Tensor, Tensor]:
        shape = x.size(0)
...
class MPMCacheDiffSim(MPMSim):
    ...
    def forward(
        self, step: int, x: Tensor, v: Tensor, C: Tensor, F: Tensor, stress: Tensor
    ) -> tuple[Tensor, Tensor, Tensor, Tensor]:
        shape = x.size(0)
