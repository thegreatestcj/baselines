...
from .loralib import mark_only_lora_as_trainable, lora_state_dict, replace_with_linear_lora
...
class MLPBlock(nn.Module):
    def __init__(
        self, in_planes: int, out_planes: int, no_bias: bool, norm: Optional[str], nonlinearity: Optional[str]
    ) -> None:
        super().__init__()
        if norm == "wn":
...
class InvariantFullMetaElasticity(MetaElasticity):
    ...
    def forward(self, F: Tensor) -> Tensor:
        ...
        if not cauchy.isfinite().all():
            print("[WARNING] cauchy stress is not finite", cauchy[~cauchy.isfinite()])
            cauchy.nan_to_num_(nan=0.0, posinf=1e8, neginf=-1e8)
        return cauchy
...
class SplineMetaElasticity(MetaElasticity):
    ...
    def get_func(self, yk, lambd):
        ...
        xi = (lambd - self.xk[indices].view_as(lambd)) / (
            self.xk[indices + 1].view_as(lambd) - self.xk[indices].view_as(lambd)
        )
    ...
    def forward(self, F: Tensor) -> Tensor:
        ...
        areas = torch.stack([sigma[:, 0] * sigma[:, 1], sigma[:, 1] * sigma[:, 2], sigma[:, 0] * sigma[:, 2]], dim=1)
...
class MetaPlasticity(Plasticity):
    def __init__(self, cfg: DictConfig) -> None:
        ...
        if cfg.param_alpha:
            self.alpha = nn.Parameter(torch.as_tensor(cfg.alpha))
        else:
...
class InvariantFullMetaPlasticity(MetaPlasticity):
    ...
    def forward(self, F: Tensor) -> Tensor:
        ...
        x = self.unflatten(x)
        x = 0.5 * (self.transpose(x) + x)
        alpha = self.alpha if isinstance(self.alpha, float) else nn.functional.elu(self.alpha, self.cfg.alpha)
        delta_Fp = alpha * torch.matmul(R, x)
        Fp = delta_Fp + F
        if not Fp.isfinite().all():
            print("[WARNING] deformation gradient is not finite", Fp[~Fp.isfinite()])
            Fp.nan_to_num_(nan=0.0, posinf=1e8, neginf=1e8)
        return Fp
