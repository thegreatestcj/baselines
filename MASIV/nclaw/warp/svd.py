...
class SVDFunction(autograd.Function):
    ...
    def backward(
        ...
        grad_U = (
            wp.zeros_like(U, requires_grad=False)
            if grad_U is None
            else wp.from_torch(grad_U.contiguous(), dtype=wp.mat33, requires_grad=False)
        )
        U.grad.assign(grad_U)
        grad_sigma = (
            wp.zeros_like(sigma, requires_grad=False)
            if grad_sigma is None
            else wp.from_torch(grad_sigma.contiguous(), dtype=wp.vec3, requires_grad=False)
        )
        sigma.grad.assign(grad_sigma)
        grad_Vh = (
            wp.zeros_like(Vh, requires_grad=False)
            if grad_Vh is None
            else wp.from_torch(grad_Vh.contiguous(), dtype=wp.mat33, requires_grad=False)
        )
