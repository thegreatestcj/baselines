...
def ceil(a, decimal=0):
    return np.round(a + 0.5 * 10 ** (-decimal), decimal)


def floor(a, decimal=0):
    return np.round(a - 0.5 * 10 ** (-decimal), decimal)


def safe_state(silent, gpu=0):
    old_f = sys.stdout
    ...
    np.random.seed(0)
    torch.manual_seed(0)
    torch.cuda.set_device(torch.device(f"cuda:{gpu}"))
