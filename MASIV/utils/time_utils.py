    ...
    def __init__(self, t_multires=6, timenet=True, time_out=30):
        ...
        if timenet:
            self.timenet = nn.Sequential(
                nn.Linear(self.time_input_ch, 256), nn.ReLU(inplace=True), nn.Linear(256, time_out)
            )
...
class MotionBasis(nn.Module):
    def __init__(self, time_input_ch, num_basis=10, D=8, W=256, num_attribute=10):
        ...
        self.linear_t = nn.ModuleList(
            ...
            + [nn.Linear(W, W) if i not in self.skips else nn.Linear(W + time_input_ch, W) for i in range(D - 1)]
        )
...
class DeformNetwork(nn.Module):
    def __init__(
        ...
        self.mb = MotionBasis(
            self.te.time_input_ch, num_basis=num_basis, W=channel_mb, D=depth_mb, num_attribute=num_attribute
        )
        self.cn = CoefficientNet(
            self.pe.xyz_input_ch,
            self.te.time_input_ch,
            num_basis=num_basis,
            num_coeff_set_per_basis=num_coeff_set_per_basis,
            W=channel_cn,
            D=depth_cn,
            softmax=softmax,
...
class Deformable3DGaussians(nn.Module):
    def __init__(self, D=8, W=256, input_ch=3, output_ch=59, multires=10, is_blender=False, is_6dof=False):
        ...
        if is_blender:
            ...
            self.timenet = nn.Sequential(
                nn.Linear(time_input_ch, 256), nn.ReLU(inplace=True), nn.Linear(256, self.time_out)
            )
